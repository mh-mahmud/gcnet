            ...
            <div class="row no-gutters position-relative">
                <div class="col-lg-3 position-static order-2 order-lg-0">
                    <div class="category-sidebar">
                        ...
                        <ul class="categories no-scrollbar d-none d-lg-block">
                            <li class="d-lg-none">
                                <a href="{{ route('categories.all') }}">
                                    <img class="cat-image lazyload" src="{{ asset('frontend/images/placeholder.jpg') }}" data-src="{{ asset('frontend/images/icons/list.png') }}" width="30" alt="{{ __('All Cagory') }}">
                                    <span class="cat-name">{{__('All')}} <br> {{__('Category')}}</span>
                                </a>
                            </li>
                            @foreach (\App\Category::all()->take(4) as $key => $category)
                                @php
                                    $brands = array();
                                @endphp
                                <li class="category-nav-element" data-id="{{ $category->id }}">
                                    <a href="{{ route('products.category', $category->id) }}">
                                        {{--<img class="cat-image lazyload" src="{{ asset($category->banner) }}" data-src="{{ asset($category->icon) }}" width="30" alt="{{ __($category->name) }}">--}}
                ...
                <div class="@if($num_todays_deal > 0) col-lg-7 @else col-lg-9 @endif order-1 order-lg-0">
                            ...
                            <div class="slick-carousel" data-slick-arrows="true" data-slick-dots="true" data-slick-autoplay="true">
                                @foreach (\App\Slider::where('published', 1)->get() as $key => $slider)
                                    <div class="" style="height:275px;">
                                        ...
                                        <img class="d-block w-100 h-100 lazyload" src="{{ asset($slider->photo) }}" data-src="{{ asset($slider->photo) }}" alt="{{ env('APP_NAME')}} promo">
                                        </a>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="trending-category  d-none d-lg-block">
                        <ul>
                            @foreach (\App\Category::where('featured', 1)->get()->take(3) as $key => $category)
                                <li @if ($key == 0) class="active" @endif>
                                    <div class="trend-category-single">
                                        ...
                                        <a href="{{ route('products.category', $category->id) }}" class="d-block">
                                            ...
                                            <div class="img">
                                                <img src="{{ asset($category->banner) }}" data-src="{{ asset($category->banner) }}" alt="{{ __($category->name) }}" class="lazyload img-fit">
                                            </div>
                                        </a>
                                    </div>
                                </li>
                                ...
                                @if ($product != null)
                                    <a href="{{ route('product', $product->slug) }}" class="d-block flash-deal-item">
                                        <div class="row no-gutters align-items-center">
                                            <div class="col">
                                                <div class="img">
                                                    <img class="lazyload img-fit" src="{{ asset($product->flash_deal_img) }}" data-src="{{ asset($product->flash_deal_img) }}" alt="{{ __($product->name) }}">
                                                </div>
                                            </div>
                                            <div class="col">
                                                <div class="price">
                    ...
                    <div class="tab-content">
                            ...
                            <div class="tab-pane fade show active">
                                ...
                                <div class="row gutters-5 sm-no-gutters">
                                    @foreach (filter_products(\App\Product::where('published', 1)->where('category_id', $homeCategory->category_id))->limit(16)->get() as $key => $product)
                ...
                <div class="col-lg-12">
                    ...
                    <div class="row">
                        @foreach (\App\Brand::where('top', 1)->get() as $brand)
                            <div class="mb-3 col-6 col-sm-3 col-xs-2">
