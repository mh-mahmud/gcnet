	...
	*{
	...
	<div style="padding: 1.5rem;">
		<table>
			...
			<tr><td class="gry-color small">{{ $shipping_address->country }}</td></tr>
        ...
        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
	        <tbody>
		        <tr>
		            ...
		            <td>{{ ($order->orderDetails->where('seller_id', Auth::user()->id)->sum('price')) }}</td>
		        </tr>

		        {{--<tr>
		            <th class="gry-color text-left">Shipping Cost</td>
		            <td>{{ ($order->orderDetails->where('seller_id', Auth::user()->id)->sum('shipping_cost')) }}</td>
		        </tr>--}}

		        <tr class="border-bottom">
		            <th class="gry-color text-left">Total Tax</td>
		            <td>{{ ($order->orderDetails->where('seller_id', Auth::user()->id)->sum('tax')) }}</td>
		        </tr>
		        <tr>
		            <th class="text-left strong">Grand Total</td>
		            <td>{{ ($order->orderDetails->where('seller_id', Auth::user()->id)->sum('price') + $order->orderDetails->where('seller_id', Auth::user()->id)->sum('tax') + $order->orderDetails->where('seller_id', Auth::user()->id)->sum('tax')) }}</td>
		        </tr>
	        </tbody>
	    </table>
    </div>
